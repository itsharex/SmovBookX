            ...
            <template #buttons>
                <vxe-button>
                    ...
                    <template #dropdowns>
                        ...
                        <vxe-button type="text" @click="insertEvent(xTable.getData(100))">插入到 100 行</vxe-button>
                        <vxe-button type="text" @click="insertEvent(xTable.getData(300))">插入到 300 行</vxe-button>
                    </template>
                </vxe-button>
                <vxe-button>
                    <template #default>删除操作</template>
                    <template #dropdowns>
                        <vxe-button type="text" @click="xTable.removeCheckboxRow()">删除选中</vxe-button>
                        <vxe-button type="text" @click="xTable.remove(xTable.getData(0))">删除第一行</vxe-button>
                        <vxe-button
                            type="text"
                            @click="xTable.remove(xTable.getData(xTable.getData().length - 1))"
                        >删除最后一行</vxe-button>
                        <vxe-button
                            type="text"
                            @click="xTable.remove(xTable.getData(100))"
