<template>
    <div class="Navigation">
        ...
        <div class="NavItemDiv">
            <navigation-item v-for="(item, index) in nav.list" :key="index" :choose="nav.choose" :name="item.name"
                :path="item.path" :index="index" @change-choose="changeChoose" :show="item.show" :ico="item.ico">
            </navigation-item>
        </div>

        <div class="QuickButton">
            <div>
                <navigation-item :choose="nav.choose" :name="'列表'" :index="-2" :show="true" @click="goSeek"
                    :class="onLoad ? 'bg-liuguang' : ''" :ico="HelpFilled">
                    <el-icon :size="17" class="loadIco" :class="onLoad ? 'onLoad' : ''" v-if="false">
            ...
            <div>
                <navigation-item :choose="nav.choose" :name="'HFS'" :index="-3" :show="true" @click.stop="hfs"
                    :class="HfsStatus == 2 ? 'bg-liuguang' : ''" :ico="Platform">
            ...
            <div class="UploadFilled">
                <el-popover placement="right" title="更新" :width="200" trigger="hover"
                    v-model:visible="UpdatePopover.show" v-if="Updater.shouldUpdate">
                    <p>检测到新的更新，点击当前按钮进行更新</p>
                    <p class="Version">版本号:{{ Updater.manifest.version }}</p>
                    <div style="text-align: right; margin: 0"></div>
                    <template #reference>
                        <navigation-item :choose="nav.choose" :name="'更新'" :index="-2" :show="true"
                            @mouseover="UpdatePopover.show = true" @mouseleave="UpdatePopover.show = false"
                            @click="install" :ico="UploadFilled"></navigation-item>
                    </template>
                </el-popover>
            </div>
        </div>
...
import { ArrowLeftBold, Loading, Download, Cloudy, HomeFilled, Menu, Tools, UploadFilled, HelpFilled, Platform, Iphone } from '@element-plus/icons-vue';
    ...
    setup(props, { emit }) {
        ...
        const hfs = () => {
            if (HfsStatus.value != 1) {
                const commond = HfsStatus.value == 0 ? "rocket_main" : "request_shutdown"
                request(commond)
                console.log("流程测试1")
                HfsStatus.value = 1
            }
