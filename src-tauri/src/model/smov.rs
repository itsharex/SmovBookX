...
impl SmovFile {
    ...
    pub fn query_db_file_unid() -> Result<Vec<SmovFile>, rusqlite::Error> {
        let mut conn = get_conn();
        let mut stmt =
            conn.prepare("SELECT realname,seekname,path,len,created,modified,extension,format FROM smov")?;
    ...
    pub fn query_unseek_db_file() -> Result<Vec<SmovFileSeek>, rusqlite::Error> {
        let mut conn = get_conn();
        let mut stmt = conn
            .prepare("SELECT id,realname,path,extension,format FROM smov where is_retrieve = 0")?;
        let smov_file_iter = stmt.query_map([], |row| {
            Ok(SmovFileSeek {
                id: row.get(0)?,
    ...
    pub fn query_db_file_id() -> Result<Vec<SmovFile>, rusqlite::Error> {
        let mut conn = get_conn();
        let mut stmt =
            conn.prepare("SELECT id,realname,seekname,path,len,created,modified,extension,format FROM smov")?;
        let smov_file_iter = stmt.query_map([], |row| {
            Ok(SmovFile {
                id : row.get(0)?,
                realname: row.get(1)?,  //当前的实际名称
