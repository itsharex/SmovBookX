...
pub fn smov_file() -> String {
    let main_path = "E:\\Videos\\Captures".to_string();
    ...
    let db_smov: HashSet<SmovFile> = SmovFile::query_db_file_unid().unwrap().into_iter().collect();
    ...
    format!( "扫描全部文件用时:{:?}ms，共扫描到{}个差异视频文件",end - begin,&smov.len())
}
